...
class fresadd0(nn.Module):
    def __init__(self, channels=32):
        super(fresadd0, self).__init__()

        # self.opspa = freup_pad(channels)
        # self.opfre = ConvBlock(48, 32, 5, 1, 2, activation=None, norm=None, bias = False)

        self.opspa = ConvBlock(7, 48, 9, 1, 4, activation='relu', norm=None, bias = False)
...
class fresadd1(nn.Module):
    def __init__(self, channels=32):
        super(fresadd1, self).__init__()
...
class fresadd2(nn.Module):
    def __init__(self, channels=32):
        super(fresadd2, self).__init__()

        # self.opspa = freup_pad(channels)
        # self.opfre = ConvBlock(48, 32, 5, 1, 2, activation=None, norm=None, bias = False)

        self.opspa = ConvBlock(32, channels, 5, 1, 2, activation=None, norm=None, bias = False)
        self.opfre = freup_pad(channels)
...
class Net(nn.Module):
    def __init__(self, num_channels, base_filter, args):
        ...
        out_channels = 4
        self.args = args
        self.head = fresadd0(48)#ConvBlock(num_channels, 48, 9, 1, 4, activation='relu', norm=None, bias = False)



        # self.body = ConvBlock(48, 32, 5, 1, 2, activation=None, norm=None, bias = False)
        self.body = fresadd1(32)

        self.output_conv =fresadd2(out_channels) #ConvBlock(32, out_channels, 5, 1, 2, activation=None, norm=None, bias = False)
